class Network(object):
    ...
    def update_mini_batch(self, mini_batch, eta):
        ...
        #initialise 0 arrays in the shape of the bias and weight arrays
        nabla_b = [np.zeros(b.shape) for b in self.biases]
        nabla_w = [np.zeros(w.shape) for w in self.weights]
        for x, y in mini_batch: #for each tuple in the mini-batch
            delta_nabla_b, delta_nabla_w = self.backprop(x, y) #calculate changes for weights and biases based on the mini-batch data
            #gradient vectors of b and w are the calculated changes 
            nabla_b = [nb+dnb for nb, dnb in zip(nabla_b, delta_nabla_b)]
            nabla_w = [nw+dnw for nw, dnw in zip(nabbla_w, delta_nabla_w)]
        #change weights and biases by nabla values scaled by eta learning rate
